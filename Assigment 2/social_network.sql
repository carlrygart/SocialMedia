...
CREATE TABLE user (user_id int(15) AUTO_INCREMENT PRIMARY KEY, first_name varchar(255) NOT NULL, last_name varchar(255) NOT NULL, birthday timestamp, email varchar(255) UNIQUE NOT NULL, sex char(1), hometown varchar(255));

CREATE TABLE work_experience ( work_experience_id int(15) AUTO_INCREMENT PRIMARY KEY, employee_id int(15) NOT NULL, employer varchar(255) NOT NULL, position varchar(255), city varchar(255), starting_time timestamp NOT NULL, ending_time timestamp DEFAULT 0, FOREIGN KEY (employee_id) REFERENCES user(user_id) ON DELETE CASCADE ON UPDATE CASCADE );
...
CREATE TABLE message (message_id int(15) AUTO_INCREMENT PRIMARY KEY, timestamp timestamp NOT NULL, text varchar(500) NOT NULL, sender_id int(15) NOT NULL, receiver_id int(15) NOT NULL, FOREIGN KEY (sender_id) REFERENCES user(user_id) ON DELETE CASCADE ON UPDATE CASCADE, FOREIGN KEY (receiver_id) REFERENCES user(user_id) ON DELETE CASCADE ON UPDATE CASCADE );
CREATE TABLE comment (comment_id int(15) AUTO_INCREMENT PRIMARY KEY, timestamp timestamp NOT NULL, text varchar(500) NOT NULL, user_id int(15) NOT NULL, message_id int(15) NOT NULL, FOREIGN KEY (user_id) REFERENCES user(user_id) ON DELETE CASCADE ON UPDATE CASCADE, FOREIGN KEY (message_id) REFERENCES message(message_id) ON DELETE CASCADE ON UPDATE CASCADE );
...
CREATE TABLE checkin_at (checkin_id int(15) AUTO_INCREMENT PRIMARY KEY, user_id int(15) NOT NULL, point_of_interest_id int(15) NOT NULL, timestamp timestamp NOT NULL, text varchar(500), FOREIGN KEY (user_id) REFERENCES user(user_id) ON DELETE CASCADE ON UPDATE CASCADE, FOREIGN KEY (point_of_interest_id) REFERENCES point_of_interest(point_of_interest_id) ON DELETE CASCADE ON UPDATE CASCADE );
